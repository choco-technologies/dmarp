...
All addresses use `dmroute_addr_t` from dmroute; MAC
...
|------------------------------------------------------------------|-----------------------------------------------------------------------|
| `dmarp_resolve(iface, ip, mac, timeout_ms)`                      | Resolve `ip` to a MAC address on `iface`. Broadcast address (255.255.255.255 or `iface`'s configured subnet broadcast): immediate, returns `FF:FF:FF:FF:FF:FF`, no frame sent, `timeout_ms` ignored. Cache hit: immediate, `timeout_ms` ignored. Cache miss: sends one ARP request, then waits (does not poll `dmnetif_receive()` itself - see [dmarp.md](dmarp.md#resolution)) up to `timeout_ms` for `dmarp_note_frame()` to observe and cache a matching reply. Returns `0` on success, `-EINVAL` (bad argument/family), `-ENODEV` (invalid `iface`), `-EIO` (request could not be sent), or `-ETIMEDOUT`. |
